...
import {
	waitForRequestsInFlight,
	areRequestsInFlight,
} from "./helpers/inFlightTracking"
...
export default (mutation, hookOptions = {}) => {
	...
	const mutationName =
		mutationAst.definitions[0].selectionSet.selections[0].name.value
	...
	return ({ input, ...options } = {}) =>
		mutate({
			...
			variables:
				args.length === 1 ? { [args[0].name.value]: input } : { input },
			...options,
			context: {
				...globalContext,
				...options.context,
